...
// register valazi_Recent_Post widget
function valazi_register_recent_post_widget() {
    register_widget( 'valazi_Recent_Post' );
}
add_action( 'widgets_init', 'valazi_register_recent_post_widget' );
 
 /**
 * Adds valazi_Recent_Post widget.
 */
class valazi_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'valazi_recent_post', // Base ID
            __( 'Valazi: Recent Post', 'valazi' ), // Name
            array( 'description' => __( 'A Recent Post Widget', 'valazi' ), ) // Args
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $title      = ! empty( $instance['title'] ) ?  $instance['title'] : __( 'Recent Posts', 'valazi' );
                    ...
                    <li>
                        <?php if( has_post_thumbnail() && $show_thumb ){ ?>
                            <a href="<?php the_permalink();?>" class="post-thumbnail">
                                <?php the_post_thumbnail( 'valazi-recent-post' );?>
                            </a>
                        <?php }?>
                            ...
                            <?php if( $show_date ) { ?>
                                <div class="entry-meta">
                                    <span class="posted-on"><a href="<?php the_permalink(); ?>">
                                        <time datetime="<?php echo get_the_date( _x( 'Y-m-d', 'recent post datetime attribute value', 'valazi' ) ); ?>"><?php echo get_the_date(_x( 'F j, Y', 'recent post datetime display', 'valazi' ) ); ?></time></a>
                                    </span>
                                </div>
                            <?php } ?>
                        </div>                        
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'valazi' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'valazi' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'valazi' ); ?></label>
        </p>
        <?php 
    }
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance['title']          = ! empty( $new_instance['title'] ) ? sanitize_text_field( $new_instance['title'] ) : __( 'Recent Posts', 'valazi' );
